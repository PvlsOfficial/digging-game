	...
	if Input.is_action_just_pressed("dig"):
		...
		if dig_cast.is_colliding():
			...
			if collider.is_in_group("terrain"):
				...
				voxel_tool.do_sphere((dig_pos * 5) - (voxel_terrain.position * 5), 1 * Singleton.shovel_level)
				voxel_terrain.save_modified_blocks()

func _unhandled_input(event):
	if event.is_action_pressed("sell"):
		Singleton.sell()
	if event.is_action_pressed("shovel_upgrade"):
		Singleton.level_up_shovel()
